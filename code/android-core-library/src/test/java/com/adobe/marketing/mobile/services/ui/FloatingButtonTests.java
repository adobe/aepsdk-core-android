...
public class FloatingButtonTests {
	...
	class TestableOnPositionChangedListener implements FloatingButtonView.OnPositionChangedListener {
	...
	public void setXYCallsOnPositionListener_When_ValidCoOrdinates() {
		...
		TestableOnPositionChangedListener onPositionChangedListener = new TestableOnPositionChangedListener();
	...
	public void buttonCallsOnTapDetected_When_OnClickCalled() {
		...
		FloatingButtonView floatingButtonViewSpy = mock(FloatingButtonView.class, withSettings().
				useConstructor(mockContext).defaultAnswer(CALLS_REAL_METHODS));

		ArgumentCaptor<View.OnClickListener> onClickListenerArgumentCaptor =
			ArgumentCaptor.forClass(View.OnClickListener.class);
		doNothing().when(floatingButtonViewSpy).setOnClickListener(onClickListenerArgumentCaptor.capture());
		TestableFloatingButtonListener floatingButtonListener = new TestableFloatingButtonListener();
	...
	public void buttonOnClickCalled_When_ButtonTapped() {
		...
		when(mockMotionEvent.getAction()).thenReturn(MotionEvent.ACTION_MOVE);
		when(mockMotionEvent.getRawX()).thenReturn(200f);
		when(mockMotionEvent.getRawY()).thenReturn(410f);//The threshold of tap detection is a displacement of < 20f
		floatingButtonViewSpy.onTouch(floatingButtonViewSpy, mockMotionEvent);
		when(mockMotionEvent.getAction()).thenReturn(MotionEvent.ACTION_UP);
		floatingButtonViewSpy.onTouch(floatingButtonViewSpy, mockMotionEvent);
		//verify
