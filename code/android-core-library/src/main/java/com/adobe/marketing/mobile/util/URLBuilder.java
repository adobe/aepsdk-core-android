...
class URLBuilder {
	...
	public String build() {
		if (StringUtils.isNullOrEmpty(this.server)) {
			Log.error("URLBuilder", "Failed to generate the URL for (server:%s,  path:%s, query:%s)",
					  this.server, this.path, this.query);
			return null;
		}

		boolean hasQuery = this.query != null && this.query.length() > 0;
		final String urlString = String.format("%s://%s%s%s%s",
