...
public class FloatingButtonView extends Button implements View.OnTouchListener {
	...
	private UIService.FloatingButtonListener buttonListener;
	 ...
	 *
	 * <p>
	 * The {@link OnClickListener} for the button calls through to the
	 * {@link UIService.FloatingButtonListener#onTapDetected()}. <br>
	 *
	 * @param listener The {@code UIService.FloatingButtonListener} instance. If this is null, the listener is cleared.
	 */
	void setFloatingButtonListener(final UIService.FloatingButtonListener listener) {
	...
	/**
	 * Sets the background image from the encoded file {@link ButtonBackground#ENCODED_BACKGROUND_PNG}
	 */
	private void setBackgroundCompat() {
		byte[] backgroundImage = Base64.decode(ButtonBackground.ENCODED_BACKGROUND_PNG, Base64.DEFAULT);
	...
	public boolean onTouch(View v, MotionEvent me) {
		if (me.getAction() == MotionEvent.ACTION_UP) {
			...
			float displacement = Math.abs(currentXvalue - oldXvalue) + Math.abs(currentYvalue - oldYvalue);
	...
	/**
	 * Set X and Y co-ordinates for the button. This is only available for API 11 and above. In lower versions, this will have not effect.
	 *
	...
	void setXYCompat(float x, float y) {
		try {
			...
			Log.error(TAG, "Error while setting the position (%s)", e);
		}
