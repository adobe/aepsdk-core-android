...
internal class LaunchRulesEvaluator(
    ...
    constructor(name: String, extensionApi: ExtensionApi) : this(name, LaunchRulesEngine(), extensionApi)

    private var cachedEvents: MutableList<Event>? = mutableListOf()
    private val logTag = "LaunchRulesEvaluator_$name"
    private val transformer: Transforming = LaunchRuleTransformer.createTransforming()
    ...
    override fun process(event: Event?): Event? {
        ...
        if (event.type == EVENT_TYPE_RULES_ENGINE && event.source == EVENT_SOURCE_REQUEST_RESET) {
            reprocessCachedEvents(launchTokenFinder)
            return event
        } else {
            cacheEvent(event)
            val matchedRules = launchRulesEngine.process(launchTokenFinder)
    ...
    private fun reprocessCachedEvents(tokenFinder: TokenFinder) {
        cachedEvents?.forEach { event ->
            launchRulesEngine.process(tokenFinder)
     ...
     */
    fun replaceToken(consequence: RuleConsequence, tokenFinder: TokenFinder): RuleConsequence {
    ...
    private fun replaceToken(value: String, tokenFinder: TokenFinder): String {
        ...
        return template.render(tokenFinder, transformer)
    }
