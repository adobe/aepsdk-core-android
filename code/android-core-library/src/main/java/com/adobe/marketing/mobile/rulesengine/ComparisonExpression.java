...
public class ComparisonExpression<A, B> implements Evaluable {
	...
	/**
	 * Initializer.
	 * Constructs the {@link ComparisonExpression} object with operands and operation name.
	 *
	 * @param lhs an {@link Operand}
	 * @param operationName A {@code String} value defining the operation.
	 * @param rhs an {@link Operand}
	 */
	public ComparisonExpression(final Operand<A> lhs, final String operationName,  final Operand<B> rhs) {
		this.lhs = lhs;
	 ...
	 * Returns RulesResult failure if the operation name or either of the operands are null.
	 *
	 * @param context The context containing details for token swapping and transforming the operand if required.
	 *                A non null value of {@link Context} is expected to be passed to this method.
	...
	public RulesResult evaluate(final Context context) {
		...
		if (resolvedLhs == null || resolvedRhs == null) {
			return new RulesResult(RulesResult.FailureType.INVALID_OPERAND, String.format("Comparison %s %s %s returned false",
