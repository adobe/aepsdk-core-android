...
public class MessageWebViewTests {
	...
	public void testCreateMessageWebView() throws IllegalArgumentException {
		...
		// test
		messageWebView = new MessageWebView(mockContext, cornerRadius, leftMargin, topMargin, rightMargin, bottomMargin);
		// verify
	...
	public void testCreateMessageWebView_WhenContextIsNull() throws IllegalArgumentException {
		// test
		MessageWebView messageWebView = new MessageWebView(null, cornerRadius, leftMargin, topMargin, rightMargin,
				bottomMargin);
		// verify
	...
	public void testMessageWebViewOnDraw() throws IllegalArgumentException {
		// setup
		MessageWebView messageWebView = new MessageWebView(mockContext, cornerRadius, leftMargin, topMargin, rightMargin,
				bottomMargin);
		// call onSizeChanged to setup a RectF for the webview
		messageWebView.onSizeChanged(messageWebView.getWidth(), messageWebView.getHeight(), messageWebView.getWidth(),
									 messageWebView.getHeight());
		// test
		messageWebView.onDraw(mockCanvas);
		// verify
		Assert.assertNotNull(messageWebView);
		Mockito.verify(mockCanvas, Mockito.times(1)).drawRoundRect(ArgumentMatchers.any(RectF.class),
				ArgumentMatchers.anyFloat(), ArgumentMatchers.anyFloat(), ArgumentMatchers.any(Paint.class));
	}
}
