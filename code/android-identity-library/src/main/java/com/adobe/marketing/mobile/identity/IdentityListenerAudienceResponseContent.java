...
/**
 * Listens for {@link EventType#AUDIENCEMANAGER}, {@link EventSource#RESPONSE_CONTENT} events and passes to parent {@link IdentityExtension}
 */
public class IdentityListenerAudienceResponseContent extends ModuleEventListener<IdentityExtension> {
	 ...
	 */
	IdentityListenerAudienceResponseContent(final IdentityExtension extension, final EventType type,
	...
	public void hear(final Event event) {
		...
		parentModule.getExecutor().execute(new Runnable() {
			@Override
			public void run() {
				parentModule.processAudienceResponse(event);
			}
		});
	}
