...
public class EventHubTest {
	...
	private boolean onEventReprocessingCompleteGetCalled = false;
	private boolean rulesFlag = false;
	@Test(timeout = 200)
	public void replaceRulesAndEvaluateEvents_happy() throws Exception {
		EVENT_LIST.clear();
		...
		consequenceEvents.add(new Event.Builder("Test1", EventType.ANALYTICS,
												EventSource.REQUEST_CONTENT).setData(eventData).build());
		consequenceEvents.add(new Event.Builder("Test2", EventType.ANALYTICS,
												EventSource.REQUEST_CONTENT).setData(eventData).build());
		consequenceEvents.add(new Event.Builder("Test3", EventType.ANALYTICS,
												EventSource.REQUEST_CONTENT).setData(eventData).build());
		rules.add(new Rule(new RuleCondition() {
			@Override
		...
		Module activeModule = eventHub.getActiveModules().iterator().next();

		eventHub.replaceRulesAndEvaluateEvents(activeModule, rules, new ReprocessEventsHandler() {
			@Override
			public List<Event> getEvents() {
				rulesFlag = true;
				List<Event> list = new ArrayList<Event>();
				list.add(new Event.Builder("Test", EventType.HUB, EventSource.NONE).build());
				return list;
			}
			@Override
			public void onEventReprocessingComplete() {
				onEventReprocessingCompleteGetCalled = true;
				rulesFlag = false;
			}
		});
		latch.await(EVENTHUB_WAIT_MS, TimeUnit.MILLISECONDS);
		assertTrue(onEventReprocessingCompleteGetCalled);
		assertEquals(5, EVENT_LIST.size());
	}
