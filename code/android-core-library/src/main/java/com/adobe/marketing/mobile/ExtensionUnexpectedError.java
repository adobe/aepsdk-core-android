...
/**
 * Wraps an unexpected asynchronous error encountered by a 3rd party extension while processing. It may contain a
 * custom message, the {@link Throwable} cause and the custom error code {@link ExtensionError}.
 *
...
public class ExtensionUnexpectedError extends Exception {
	...
	/**
	 * Constructs an {@code ExtensionUnexpectedError} with a detailed message, cause, and error code.
	 * @param message a {@link String} detailed message
	 * @param cause the {@link Throwable} cause
	 * @param code the {@link ExtensionError} code
	 */
	ExtensionUnexpectedError(final String message, final Throwable cause, final ExtensionError code) {
		super(message, cause);
		this.errorCode = code;
	}
