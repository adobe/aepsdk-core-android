...
internal object MobileIdentitiesProvider {
     ...
     * A state is ready if [SharedStateResult.status] is not [SharedStateStatus.PENDING]
     * The required shared states are [REQUIRED_READY_EXTENSIONS]
     *
     * @param event an [Event] generated by the GetSdkIdentities API
     * @param extensionApi extensionApi
     * @return true, if the all the shared states required are not [SharedStateStatus.PENDING];
     *         false, if any one of the shared states required [SharedStateResult.PENDING]
     */
    internal fun areAllSharedStatesReady(event: Event, extensionApi: ExtensionApi): Boolean {
        var ready = true
        REQUIRED_READY_EXTENSIONS.forEach { extension ->
            ready = ready && getSharedState(
                extension,
                event,
                extensionApi
    ...
    internal fun collectSdkIdentifiers(event: Event, extensionApi: ExtensionApi): String {
        ...
        identifiers.forEach { userId ->
            identifierMaps.add(
                mapOf(
                    JSON_KEY_NAMESPACE to userId.namespace,
                    JSON_KEY_VALUE to userId.value,
                    JSON_KEY_TYPE to userId.type
                )
            )
        }
