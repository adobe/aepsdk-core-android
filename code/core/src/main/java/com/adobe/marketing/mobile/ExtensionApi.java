...
public abstract class ExtensionApi {
     ...
     * @param enforceOrder if `true`, consecutive lookups will use the oldest timestamp from the *
     *     previous event as their from date
     * @param handler contains an {@link EventHistoryResult} for each provided request
     */
    public abstract void getHistoricalEvents(
            @NonNull EventHistoryRequest[] eventHistoryRequests,
            boolean enforceOrder,
     ...
     * @param handler the {@link AdobeCallback} which returns a boolean indicating a successful
     *     database insert
     */
    public abstract void recordHistoricalEvent(
            @NonNull final Event event, @NonNull final AdobeCallback<Boolean> handler);
}
