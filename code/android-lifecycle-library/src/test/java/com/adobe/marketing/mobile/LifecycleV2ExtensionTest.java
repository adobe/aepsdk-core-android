...
public class LifecycleV2ExtensionTest {
	...
	public void beforeEach() {
		...
		lifecycleV2 = new LifecycleV2Extension(lifecycleDataStore, mockDeviceInfoService, extensionApi);
	...
	public void start_happy_regularLaunch() {
		...
		verify(lifecycleDataStore, times(1)).setLong(eq(DATASTORE_KEY_APP_START_TIMESTAMP_MILLIS) ,appStartTimestampCaptor.capture());
		assertEquals(currentTimestampInMilliseconds, appStartTimestampCaptor.getValue().longValue());
		...
		verify(mockBuilder, times(1)).buildAppLaunchXDMData(launchTimestampCaptor.capture(),
		...
		verify(mockBuilder, never()).buildAppCloseXDMData(anyLong(),
				anyLong(),
				anyLong(),
				anyBoolean());
	}
	...
	public void start_consecutiveStartEvents_updatesOnlyFirstTime() {
		...
		ArgumentCaptor<Long> appStartTimestampCaptor = ArgumentCaptor.forClass(Long.class);
		verify(lifecycleDataStore, times(1)).setLong(eq(DATASTORE_KEY_APP_START_TIMESTAMP_MILLIS) ,appStartTimestampCaptor.capture());
		assertEquals(timestampTwoSecondsEarlierMilliseconds, appStartTimestampCaptor.getValue().longValue());
		...
		assertEquals(timestampTwoSecondsEarlierMilliseconds, launchTimestampCaptor.getValue().longValue());
		assertFalse(isInstallCaptor.getValue());
	...
	public void start_onInstall_doesNotBuildCloseEvent() {
		...
		verify(mockBuilder, times(1)).buildAppLaunchXDMData(anyLong(),
				anyBoolean(),
				anyBoolean()
		);
	...
	public void start_onCloseUnknown_missingPause_buildsCloseEvent() {
		...
		ArgumentCaptor<Long> fallbackCloseEventTimestampCaptor = ArgumentCaptor.forClass(Long.class);
		ArgumentCaptor<Boolean> isCloseKnownCaptor = ArgumentCaptor.forClass(Boolean.class);
		verify(mockBuilder, times(1)).buildAppCloseXDMData(launchTimestampCaptor.capture(),
				closeTimestampCaptor.capture(),
				fallbackCloseEventTimestampCaptor.capture(),
				isCloseKnownCaptor.capture());

		assertEquals(timestampOneHourEarlierInMilliseconds, launchTimestampCaptor.getValue().longValue());
	...
	public void start_onCloseUnknown_startAfterPause_buildsCloseEvent() {
		...
		assertEquals(timestampTenMinEarlierInMilliseconds, launchTimestampCaptor.getValue().longValue());
		assertEquals(timestampTenMinEarlierInMilliseconds + CLOSE_TIMESTAMP_OFFSET_MILLIS,
				closeTimestampCaptor.getValue().longValue());
		assertEquals(currentTimestampInMilliseconds - 1000,
				fallbackCloseEventTimestampCaptor.getValue().longValue()); // backdated start time
	...
	public void pause_Happy() throws Exception {
		...
		Event testEvent = new Event.Builder(null, TYPE_LIFECYCLE, TYPE_REQUEST_CONTENT)
		.setTimestamp(currentTimestampInMilliseconds)
		.build();
		...
		verify(lifecycleDataStore, times(1)).setLong(eq(DATASTORE_KEY_APP_PAUSE_TIMESTAMP_MILLIS),
				appPauseTimestampCaptor.capture());
		assertEquals(currentTimestampInMilliseconds, appPauseTimestampCaptor.getValue().longValue());

		verify(mockBuilder, never()).buildAppLaunchXDMData(anyLong(),
				anyBoolean(),
				anyBoolean()
		);
		...
		assertEquals(currentTimestampInMilliseconds, fallbackCloseEventTimestampCaptor.getValue().longValue()); // backdated start time
	...
	public void updateLastKnownTimestamp_Happy() {
		...
		verify(lifecycleDataStore, times(1)).setLong(eq(DATASTORE_KEY_APP_CLOSE_TIMESTAMP_MILLIS),
				appCloseTimestampCaptor.capture());
		assertEquals(currentTimestampInMilliseconds, appCloseTimestampCaptor.getValue().longValue());
	}

	private void mockPersistence(final long startTimeSec, final long pauseTimeSec, final long closeTimeSec,
                                 final boolean isUpgrade) {
        when(lifecycleDataStore.getLong(eq(DATASTORE_KEY_APP_START_TIMESTAMP_MILLIS), anyLong())).thenReturn(startTimeSec);
        when(lifecycleDataStore.getLong(eq(DATASTORE_KEY_APP_PAUSE_TIMESTAMP_MILLIS), anyLong())).thenReturn(pauseTimeSec);
        when(lifecycleDataStore.getLong(eq(DATASTORE_KEY_APP_CLOSE_TIMESTAMP_MILLIS), anyLong())).thenReturn(closeTimeSec);

        when(lifecycleDataStore.getLong(eq(DATASTORE_KEY_INSTALL_DATE), anyLong())).thenReturn(timestampOneDayEarlierMilliseconds);
        when(lifecycleDataStore.getString(eq(DATASTORE_KEY_LAST_APP_VERSION), anyString())).thenReturn("1.0");

        if (!isUpgrade) {
            when(lifecycleDataStore.getString(eq(DATASTORE_KEY_LAST_APP_VERSION), anyString())).thenReturn("1.1");
		}
	}
