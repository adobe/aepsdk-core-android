...
@PrepareForTest(RemoteDownloader::class, FileMetadataProvider::class, FileUtils::class)
class ConfigurationDownloaderTest {
    ...
    fun `Download invokes remote downloader `() {
        ...
        `when`(FileUtils.readAsString(mockDownloadedFile)).thenReturn(mockConfigJson)
    ...
    fun `Download when RemoteDownload result file cannot be parsed`() {
        PowerMockito.mockStatic(FileUtils::class.java)
        `when`(FileUtils.readAsString(mockDownloadedFile)).thenReturn(null)
    ...
    fun `Download when RemoteDownload result file is empty`() {
        PowerMockito.mockStatic(FileUtils::class.java)
        `when`(FileUtils.readAsString(mockDownloadedFile)).thenReturn("")
    ...
    fun `Download when RemoteDownload result file has valid config content`() {
        val mockConfigContentJson = "{" +
            ...
            "}"
        PowerMockito.mockStatic(FileUtils::class.java)
        `when`(FileUtils.readAsString(mockDownloadedFile)).thenReturn(mockConfigContentJson)
