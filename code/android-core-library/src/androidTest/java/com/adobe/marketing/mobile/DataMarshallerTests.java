...
public class DataMarshallerTests {
	...
	public void marshalDeepLinkData() {
		...
		assertEquals("android-app://deeplink.uri?hello=world&goodnight=moon", String.valueOf(result.get(DEEPLINK_KEY)));
	}
	...
	public void marshalDeepLinkDataWithDuplicateParameters() {
		...
		assertEquals(1, result.size());
		assertTrue(result.containsKey(DEEPLINK_KEY));
		assertEquals("android-app://deeplink.uri?hello=world&product=xyz&product=abc",
	...
	public void marshalDeepLinkDataWithAdobeQueryParamsUpdatesIntent() {
		...
		assertEquals(1, result.size());
		assertTrue(result.containsKey(DEEPLINK_KEY));
		assertEquals("android-app://deeplink.uri?a.deeplink.id=123&hello=world&at_preview_token=a&at_preview_endpoint=b",
					 String.valueOf(result.get(DEEPLINK_KEY)));
	...
	public void marshalDeepLinkDataWithAdobeQueryParamsAndDuplicateKeysUpdatesIntent() {
		...
		assertEquals(1, result.size());
		assertTrue(result.containsKey(DEEPLINK_KEY));
		assertEquals("android-app://deeplink.uri?a.deeplink.id=123&product=xyz&product=abc&at_preview_token=a&at_preview_endpoint=b",
					 String.valueOf(result.get(DEEPLINK_KEY)));

		// call to marshal updates intent data by filtering out Adobe query parameters
		Intent cleanedIntent = activity.getIntent();
		assertEquals("android-app://deeplink.uri?product=xyz&product=abc", cleanedIntent.getDataString());
	}
