...
public class WebViewGestureListenerTests {
	...
	public void setup() throws Exception {
		...
		PowerMockito.doCallRealMethod().when(mockMessageFragment).isDismissedWithGesture();
		PowerMockito.mockStatic(ObjectAnimator.class);
		PowerMockito.mockStatic(Animator.AnimatorListener.class);
		PowerMockito.when(ObjectAnimator.class, "ofFloat", ArgumentMatchers.any(MessageWebView.class),
						  ArgumentMatchers.anyString(), ArgumentMatchers.anyFloat(),
						  ArgumentMatchers.anyFloat()).thenReturn(mockObjectAnimator);

		Whitebox.setInternalState(mockAEPMessage, "fullScreenMessageDelegate", mockFullscreenMessageDelegate);
		Whitebox.setInternalState(mockAEPMessage, "webView", mockMessageWebView);
		Whitebox.setInternalState(mockAEPMessage, "baseRootViewHeight", 3000);
		Whitebox.setInternalState(mockAEPMessage, "baseRootViewWidth", 2000);
		Whitebox.setInternalState(mockMessageFragment, "gestures", gestureMap);
		Whitebox.setInternalState(mockMessageFragment, "message", mockAEPMessage);
