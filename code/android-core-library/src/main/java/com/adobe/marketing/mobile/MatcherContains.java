...
/**
 * A rule condition matcher which evaluates if a given value contains any of this matcher's {@code values}.
 */
class MatcherContains extends Matcher {
	 ...
	 * @return true if {@code value} contains any of this {@code Matcher}'s values
	 */
	@Override
	protected boolean matches(final Object value) {
		...
		for (Object v : values) {
			if (v instanceof String &&
					Pattern.compile(Pattern.quote(v.toString()), Pattern.CASE_INSENSITIVE).matcher(valueToSearchFor).find()) {
				return true;
			}
		}
	...
	@Override public String toString() {
		...
		for (Object value : values) {
			...
			matcherStringBuilder.append(" CONTAINS ");
