...
class CacheFileManager {
    ...
    /**
     * Creates a directory with name {@code cacheName} inside the {@code cacheRoot}
     * folder of {@code deviceInfoService.getApplicationCacheDir()}
     *
    ...
    File createCache(final String cacheName) {
        ...
        final File cacheBucket = new File(appCacheDir, rootCacheDirName + File.separator + cacheName);
     ...
     *
     * @param cacheName the cache sub-folder within {@code cacheRoot} where
     *                  cache file stored for key should be fetched from
     ...
     *
     * @param cacheName the cache sub-folder within {@code cacheRoot} where the metadata for
     *                  cache file stored for key should be fetched from
    ...
    Map<String, String> getCacheMetadata(final String cacheName, final String key) {
        ...
        try {
            ...
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Cannot create cache metadata for" +
                    "cache name: [%s], cache key: [%s] due to %s", cacheName, key, e.getMessage());
            return null;
        }
    }

    /**
     * Creates a cache file and a cache metadata file based on {@code cacheEntry} to
     * be retrievable via {@code key}. These files are stored inside  {@code cacheRoot/cacheName} dir.
     *
    ...
    boolean createCacheFile(final String cacheName, final String key, final CacheEntry cacheEntry) {
        ...
        final boolean saved = FileUtils.readInputStreamIntoFile(new File(entryLocation),
    ...
    /**
     * Creates a metadata file based on {@code cacheEntry} to
     * be retrievable via {@code key}.
     *
     ...
     * @return true if the cache file and metadata file are created successfully; false otherwise.
     */
    private boolean createCacheMetadataFile(final CacheEntry cacheEntry,
        ...
        if (expiry != null) {
            metadata.put(FileCacheResult.METADATA_KEY_EXPIRY_IN_MILLIS, String.valueOf(expiry.getTime()));
        }
        ...
        try {
            ...
            final InputStream metadataStream =
                    new ByteArrayInputStream(metadataJson.toString().getBytes(StandardCharsets.UTF_8));
            return FileUtils.readInputStreamIntoFile(new File(metadataFilePath),
     ...
     *
     * @param cacheName the cache sub-folder from which files associated with the key need to be deleted
    ...
    boolean deleteCacheFile(final String cacheName, final String key) {
        ...
        if (cacheFileToDelete == null) {
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Cannot delete cache file. No file to delete.");
            return true;
        }

        if (!FileUtils.deleteFile(cacheFileToDelete, true)) {
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Failed to delete cache file for " +
                    "cache name [%s], key: [%s]", cacheName, key);
            return false;
        }

        final String cacheFileMetadataPath = getMetadataLocation(cacheName, key);
        if (cacheFileMetadataPath != null) {
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Failed to delete cache metadata file for " +
                    "cache name [%s], key: [%s]", cacheName, key);
