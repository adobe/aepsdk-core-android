...
public class LifecycleSessionTest {
	...
	public void start_FirstLaunchSession() {
        ...
        verify(dataStore, times(1)).setLong(eq(DATASTORE_KEY_START_DATE) ,startDateCaptor.capture());
        ...
        verify(dataStore, times(1)).setBoolean(eq(DATASTORE_KEY_SUCCESSFUL_CLOSE), successfulCloseCaptor.capture());
	...
	public void start_verifyAppIdAndOsVersion() {
		...
		session.start(currentTimestampInSeconds, sessionTimeoutInSeconds,
				coreData);

		ArgumentCaptor<String> osVersionCaptor = ArgumentCaptor.forClass(String.class);
		verify(dataStore, times(1)).setString(eq(DATASTORE_KEY_OS_VERSION), osVersionCaptor.capture());
	...
	public void start_ResumeSession() {
		...
		when(dataStore.getLong(eq(DATASTORE_KEY_START_DATE), anyLong())).thenReturn(currentTimestampInSeconds);
		when(dataStore.getLong(eq(DATASTORE_KEY_PAUSE_DATE), anyLong())).thenReturn(previousSessionPauseTimestamp);
		when(dataStore.getBoolean(eq(DATASTORE_KEY_SUCCESSFUL_CLOSE), anyBoolean())).thenReturn(true);
		...
		ArgumentCaptor<Long> startDateCaptor = ArgumentCaptor.forClass(Long.class);
		verify(dataStore, times(1)).setLong(eq(DATASTORE_KEY_START_DATE) ,startDateCaptor.capture());
		assertEquals(previousSessionStartTimestamp + previousSessionPauseTime, startDateCaptor.getValue().longValue());
	...
	public void pause_Happy() {
		...
		verify(dataStore, times(1)).setLong(eq(DATASTORE_KEY_PAUSE_DATE) ,pauseDateCaptor.capture());
