...
final class ConfigurationSharedStateIdentity {
    ...
    void getConfigurationProperties(final Map<String, Object> sharedState) {
        ...
        this.orgID = DataReader.optString(sharedState, IdentityConstants.JSON_CONFIG_ORGID_KEY, null);
        this.marketingCloudServer = DataReader.optString(sharedState, IdentityConstants.JSON_EXPERIENCE_CLOUD_SERVER_KEY, Defaults.SERVER);

        if (StringUtils.isNullOrEmpty(this.marketingCloudServer)) {
            this.marketingCloudServer = Defaults.SERVER;
        }

        this.privacyStatus = MobilePrivacyStatus.fromString(DataReader.optString(
                sharedState,
