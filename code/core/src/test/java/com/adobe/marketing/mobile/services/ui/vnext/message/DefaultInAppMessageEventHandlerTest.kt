...
class DefaultInAppMessageEventHandlerTest {
    ...
    @Test
    fun `Test that #evaluateJavascript bails on empty js content`() {
        val scriptHandlers: MutableMap<String, DefaultInAppMessageEventHandler.WebViewJavascriptInterface> =
            mutableMapOf(
                "handler1" to scriptHandler1,
                "handler2" to scriptHandler2
        ...
        runTest {
            ...
            defaultInAppMessageEventHandler.evaluateJavascript("")
            verify(mockWebView, never()).evaluateJavascript(anyString(), any())
        }
    }

    @Test
    fun `Test that #evaluateJavascript bails when no script handlers are present`() {
        val scriptHandlers: MutableMap<String, DefaultInAppMessageEventHandler.WebViewJavascriptInterface> =
            mutableMapOf()
        val defaultInAppMessageEventHandler = DefaultInAppMessageEventHandler(
            scriptHandlers,
        ...
        runTest {
            ...
            defaultInAppMessageEventHandler.evaluateJavascript("test")
            verify(mockWebView, never()).evaluateJavascript(anyString(), any())
        }
    }

    @Test
    fun `Test thar #evaluateJavascript bails when webview is null`() {
        val scriptHandlers: MutableMap<String, DefaultInAppMessageEventHandler.WebViewJavascriptInterface> =
            mutableMapOf(
                "handler1" to scriptHandler1,
                "handler2" to scriptHandler2
        ...
        runTest {
            defaultInAppMessageEventHandler.evaluateJavascript("test")
            verify(scriptHandler1, never()).run(anyString())
            verify(scriptHandler2, never()).run(anyString())
        }
    }

    @Test
    fun `Test that #evaluateJavascript evaluates on all script handlers`() {
        val scriptHandlers: MutableMap<String, DefaultInAppMessageEventHandler.WebViewJavascriptInterface> =
            mutableMapOf(
                "handler1" to scriptHandler1,
                "handler2" to scriptHandler2
        ...
        runTest {
            ...
            verify(scriptHandler1).run("EvaluatedValue")
            verify(scriptHandler2).run("EvaluatedValue")
        }
    }
