 ...
 *
 * Once a token is identified in any part of the rule, use this class to obtain the resolved value of the token.
...
class MustacheToken {
	 ...
	 *
	 * This constructor automatically recognizes the token type from the provided token string
	 *
	...
	protected Object resolve(final TokenFinder tokenFinder, final Transforming transformers) {
		if (tokenType == Type.FUNCTION) {
			return transformers.transform(this.functionName, innerVariable.resolve(tokenFinder, transformers));
		} else {
