...
/**
 * An extension of {@link WebView} that customizes the drawing of the {@link Canvas} to allow for rounded corners.
 */
class MessageWebView extends WebView {
	 ...
	 * @param cornerRadius a {@code float} defining the angle to use when rounding the message's webview.
	 ...
	 */
	public MessageWebView(final Context context, final float cornerRadius, final int left, final int top, final int right,
						  final int bottom) throws IllegalArgumentException {
		super(context);
	...
	/**
	 * Initializes the {@link Paint}, {@link Bitmap}, and {@link BitmapShader} objects needed to add the rounded corners to the webview.
	 */
