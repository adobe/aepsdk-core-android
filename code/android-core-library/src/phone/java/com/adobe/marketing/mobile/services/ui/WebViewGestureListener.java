...
/**
 * Listens for {@link MotionEvent}s and determines if a swipe gesture occurred on the {@link MessageWebView}.
 */
class WebViewGestureListener extends GestureDetector.SimpleOnGestureListener {
	...
	/**
	 * Invoked when a fling event is detected after an initial "onDown" MotionEvent and a matching "up" MotionEvent.
	 ...
	 */
	@Override
	public boolean onFling(final MotionEvent motionEvent, final MotionEvent motionEvent2, final float velocityX,
	...
	/**
	 * Generates a dismiss animation using the {@link ObjectAnimator}. The {@link MessageWebView} will be dismissed at the
	 * direction of the detected swipe {@link MessageGesture}.
	 * If the webview was dismissed via a {@code MessageGesture.BACKGROUND_TAP} then the {@code MessageWebView} will be dismissed
	 * using the dismissal {@link MessageAnimation} specified in the {@link MessageSettings}.
	 *
	...
	/**
	 * Dismisses the {@link MessageWebView} by calling the {@link FullscreenMessageDelegate#overrideUrlLoad} function
	 * present in the parent {@link MessageFragment}'s message listener.
	 *
	 * @param gesture The detected {@code MessageGesture} that occurred.
	 * @param dismissedWithGesture true if a swipe gesture occurred, false if a background tap occurred.
	 */
	private void dismissMessage(final MessageGesture gesture, final boolean dismissedWithGesture) {
		...
		if (parentFragment.message != null && parentFragment.message.fullScreenMessageDelegate != null) {
			final String behavior = parentFragment.gestures == null ? null : parentFragment.gestures.get(gesture);

			// if we have a gesture mapping with behaviors, use the specified behavior. otherwise, just dismiss the message.
			if (!StringUtils.isNullOrEmpty(behavior)) {
				parentFragment.message.fullScreenMessageDelegate.overrideUrlLoad(parentFragment.message, behavior);
			} else {
