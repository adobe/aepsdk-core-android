...
class RulesZipProcessingHelper {
    ...
    boolean createTemporaryRulesDirectory(final String tag) {
        ...
        if (!tempExtractDir.exists() && !tempExtractDir.mkdirs()) {
            Log.debug(CoreConstants.LOG_TAG, TAG, "Cannot access application cache directory to create temp dir.");
            return false;
        }
        return true;
    }
    ...
    String unzipRules(final String tag) {
        ...
        if (!extracted) {
            Log.debug(CoreConstants.LOG_TAG, TAG, "Failed to extract rules response zip into temp dir.");
            return null;
        }
