...
public class AndroidUIService implements UIService {
	...
	protected Intent getIntentWithUrl(String url) {
		...
		Intent intent = null;
		if (handler != null){
			intent = handler.uriTarget(url);
			if (intent == null){
				MobileCore.log(LoggingMode.DEBUG, LOG_TAG,  String.format("%s is not handled with a custom Intent, use SDK's default Intent instead.", url));
			}
		}
		if (intent == null){
