...
@PrepareForTest(RemoteDownloader::class, ZipFileMetadataProvider::class, FileUtils::class)
class ConfigurationRulesDownloaderTest {
    ...
    fun setUp() {
        configurationRulesDownloader =
            ConfigurationRulesDownloader(mockNetworking, mockCacheFileService, mockMetadataProvider)
        Whitebox.setInternalState(
            configurationRulesDownloader,
            "remoteDownloader",
            mockRemoteDownloader
        )
    }
    ...
    fun `Process rules when downloaded zip is a directory`() {
        ...
        callbackCaptor.firstValue.invoke(mockDownloadedFile)
    ...
    fun `Process rules zip when downloaded zip extraction fails`() {
        ...
        `when`(FileUtils.extractFromZip(mockDownloadedFile, VALID_OUTPUT_PATH)).thenReturn(false)
    ...
    fun `Process rules zip when downloaded zip extraction succeeds`() {
        ...
        PowerMockito.mockStatic(FileUtils::class.java)
        `when`(FileUtils.extractFromZip(mockDownloadedFile, VALID_OUTPUT_PATH)).thenReturn(true)
