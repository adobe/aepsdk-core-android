...
public class HttpConnectingHandlerTests {
	...
	@Mock
	private HttpsURLConnection httspURLConnection;
	...
	public void testSetCommand_Get_Method() throws IOException {
		...
		}).when(httspURLConnection).setDoOutput(ArgumentMatchers.anyBoolean());
		//Test
	...
	public void testSetCommand_Protocol_Exception() throws IOException {
		...
		final HttpMethod testMethod = HttpMethod.GET;
		final AtomicBoolean actualDoOutputValue = new AtomicBoolean(false);
		Mockito.doThrow(new ProtocolException("mockException")).when(httspURLConnection).setRequestMethod(
	...
	public void testSetRequestProperty_Valid_Map_Parameter() throws IOException {
		...
		}).when(httspURLConnection).setRequestProperty(ArgumentMatchers.anyString(), ArgumentMatchers.anyString());
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
		HashMap<String, String> testPropertyList = new HashMap<String, String>() {
			{
				put("key1", "value1");
				put("key2", "value2");
			}
	...
	public void testSetRequestProperty_Cumulative_Operations() throws IOException {
		...
		//Valid Map input
		connectionHandler.setRequestProperty(testPropertyList);
		HashMap<String, String> testPropertyList1 = new HashMap<String, String>() {
			{
				put("key3", "value3");
				put("key1", "value4");
			}
	...
	public void testConnect() throws IOException {
		...
		}).when(httspURLConnection).connect();
		//Test
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
		HttpConnection connection =
			(HttpConnection) connectionHandler.connect(null);
		//Verify
	...
	public void testConnect_Post_Command_With_Valid_Payload() throws IOException {
		...
		}).when(httspURLConnection).setFixedLengthStreamingMode(ArgumentMatchers.anyInt());
		//Output Stream
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		...
		//POST with no payload
		HttpConnection connection =
			(HttpConnection) connectionHandler.connect(payload.getBytes());
		//Verify
		Assert.assertTrue(connectCalled.get());
		Assert.assertNotNull(connection);
	...
	public void testSetReadTimeout() throws IOException {
		...
		}).when(httspURLConnection).setReadTimeout(ArgumentMatchers.anyInt());
		//Test
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
	...
	public void testSetConnectTimeout() throws IOException {
		...
		}).when(httspURLConnection).setConnectTimeout(ArgumentMatchers.anyInt());
		//Test
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
	...
	public void testSSLSocketFactory_UseTLSSocketFactory_When_APIIsLowerThan20() throws Exception {
		...
		}).when(httspURLConnection).setSSLSocketFactory(ArgumentMatchers.any(SSLSocketFactory.class));
		//Test
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
		//Verify
		Assert.assertEquals(TLSSocketFactory.getInstance(), sslSocketFactoryList.get(0));
		Mockito.verify(httspURLConnection, Mockito.times(1)).setSSLSocketFactory(ArgumentMatchers.any(SSLSocketFactory.class));
	...
	public void testSSLSocketFactory_NotUseTLSSocketFactory_When_APIEquals20() throws Exception {
		...
		HttpConnectionHandler connectionHandler = new HttpConnectionHandler(url);
		//Verify
		Mockito.verify(httspURLConnection, Mockito.times(0)).setSSLSocketFactory(ArgumentMatchers.any(SSLSocketFactory.class));
	}
