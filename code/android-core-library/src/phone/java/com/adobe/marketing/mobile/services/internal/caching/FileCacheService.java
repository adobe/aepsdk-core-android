...
public class FileCacheService implements CacheService {
     ...
     * Stores the content and its metadata as separate files internally in the
     * cache directory supplied by {@code DeviceInforming#getApplicationCacheDir}
     *
    ...
    public boolean set(@NonNull String cacheName, @NonNull String key, @NonNull CacheEntry value) {
        ...
        if (cacheBucket == null) {
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Could not set value for key: [%s] in cache: [%s]." +
    ...
    /**
     * Retrieves the value associated with the key being queried.
     * Note that the {@code CacheResult} returned may have additional metadata
     * internally used by the {@code FileCacheService}.
     *
     * @param cacheName name of the bucket where the entry is to be fetched from
     * @param key       key for the cache entry
     * @return the {@code FileCacheResult} associated with the key if present;
     * null otherwise.
     */
    @Nullable
    @Override
    public CacheResult get(@NonNull String cacheName, @NonNull String key) {
        ...
        // We lost the metadata/metadata is corrupt, but have an entry - so there must have been an error.
        // Remove the cache entry as well and return null.
        if (cacheMetadata == null) {
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Could not find metadata for key: [%s] in cache: [%s].");
        ...
        if (expiry.isExpired()) {
            ...
            // and return null.
            Log.debug(ServiceConstants.LOG_TAG, TAG, "Cache entry for key: [%s] in cache: [%s] has expired.");
            remove(cacheName, key);
            return null;
        }
