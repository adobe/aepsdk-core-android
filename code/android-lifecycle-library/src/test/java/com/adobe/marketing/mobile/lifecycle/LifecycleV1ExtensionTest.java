...
public class LifecycleV1ExtensionTest {
    ...
    public void testStart_PreviousSessionInfoNull_DefaultStartDate() {
        ...
        try (MockedConstruction<LifecycleState> ignored = mockConstruction(LifecycleState.class,
                (mock, context) -> {
                    when(mock.start(startTimeCaptor.capture(),
                            additionalDataCaptor.capture(),
                            adIdCaptor.capture(),
                            sessionTimeoutCaptor.capture(),
                            isInstallCaptor.capture())).thenReturn(null);
                    when(mock.getContextData()).thenReturn(null);
        })){

            LifecycleV1Extension lifecycleV1Extension = new LifecycleV1Extension(lifecycleDataStore,
            ...
            ArgumentCaptor<Map<String, Object>> lifecycleSharedState = ArgumentCaptor.forClass(Map.class);
            ArgumentCaptor<Event> lifecycleEventCaptor = ArgumentCaptor.forClass(Event.class);
            verify(extensionApi, times(1)).createSharedState(
                    lifecycleSharedState.capture(),
                    lifecycleEventCaptor.capture());
            ...
            assertEquals(MAX_SESSION_LENGTH_SECONDS, lifecycleSharedState.getValue().get(MAX_SESSION_LENGTH));
    ...
    public void testStart_PreviousSessionInfoNull_StartDateAndLifecycleContextDataSet() {
        ...
        try (MockedConstruction<LifecycleState> ignored = mockConstruction(LifecycleState.class,
            ...
            verify(extensionApi, times(1)).createSharedState(
                    lifecycleSharedState.capture(),
                    lifecycleStartEventCaptor.capture());
    ...
    public void testStart_PreviousSessionInfoNotNull() {
        ...
        try (MockedConstruction<LifecycleState> ignored = mockConstruction(LifecycleState.class,
                (mock, context) -> {
                    LifecycleSession.SessionInfo sessionInfo = new LifecycleSession.SessionInfo(111L, 222L, false);

                    when(mock.start(startTimeCaptor.capture(),
                            additionalDataCaptor.capture(),
                            adIdCaptor.capture(),
                            sessionTimeoutCaptor.capture(),
                            isInstallCaptor.capture())).thenReturn(sessionInfo);
                    when(mock.getContextData()).thenReturn(contextData);
                })){
            ...
            verify(extensionApi, times(1)).createSharedState(
                    ...
                    lifecycleStartEventCaptor.capture());
            assertEquals(lifecycleStartEvent, lifecycleStartEventCaptor.getValue());
            assertEquals(TimeUnit.MILLISECONDS.toSeconds(currentTimestampInMilliSeconds), lifecycleSharedState.getValue().get(SESSION_START_TIMESTAMP));
            assertEquals(MAX_SESSION_LENGTH_SECONDS, lifecycleSharedState.getValue().get(MAX_SESSION_LENGTH));
            assertEquals(contextData, lifecycleSharedState.getValue().get(LIFECYCLE_CONTEXT_DATA));
            ...
            Map<String, Object> lifecycleResponseEventData = lifecycleResponseEvent.getValue().getEventData();
            ...
            assertEquals(TimeUnit.MILLISECONDS.toSeconds(currentTimestampInMilliSeconds), lifecycleResponseEventData.get(SESSION_START_TIMESTAMP));
            assertEquals(MAX_SESSION_LENGTH_SECONDS, lifecycleResponseEventData.get(MAX_SESSION_LENGTH));
    ...
    public void testStart_AdditionalData() {
        ...
        try (MockedConstruction<LifecycleState> ignored = mockConstruction(LifecycleState.class,
            ...
            Event lifecycleStartEvent = createStartEvent(additionalContextData, currentTimestampInMilliSeconds);
            lifecycleV1Extension.start(lifecycleStartEvent, configurationSharedState, true);
