...
public class EventDataUtilsTests {
    ...
    @Test
    public void testClone_SimpleObjects_SkipUnsupported() throws CloneFailedException {
        Map<String, Object> values = new HashMap<>();
        values.put("boolean", true);
        ...
        Map<String, Object> clonedValues = EventDataUtils.clone(values, true);
        assertEquals(values, clonedValues);

        // No types to skip so should be the same as clone with skip
        assertEquals(clonedValues, EventDataUtils.clone(values, false));
    }

    @Test
    public void testClone_NestedObjects_SkipUnsupported() throws CloneFailedException {
        Map<String, Object> nestedMap = new TreeMap<>();
        nestedMap.put("integer", 1);
        nestedMap.put("float", 1f);
        nestedMap.put("double", 1d);
        ...
        Map<String, Object> clonedData = EventDataUtils.clone(data, true);
        assertEquals(data, clonedData);

        // No types to skip so should be the same as clone with skip
        assertEquals(clonedData, EventDataUtils.clone(data, false));
    }

    @Test
    public void testClone_MapWithNonStringKeys_SkipUnsupported() throws CloneFailedException {
        Map<Object, Object> nestedMap = new HashMap<>();
        nestedMap.put(null, 1);
        nestedMap.put(new Integer(1), 1);
        nestedMap.put(new Double(1.1), 1d);
    ...
    @Test
    public void testClone_Array_SkipUnsupported() throws CloneFailedException {
        String[] stringArray = new String[] {"string1", "string2"};
        ...
        // No types to skip so should be the same as clone with skip
        Map<String, Object> clonedDataNoSkip = EventDataUtils.clone(data, false);
        assertEquals(clonedDataNoSkip.get("stringArray"), Arrays.asList(stringArray));
        assertEquals(clonedDataNoSkip.get("mapArray"), Arrays.asList(mapArray));
    }

    @Test
    public void testClone_UnsupportedTypes_SkipUnsupported() {
        class Data {}
        ...
        try {
            assertEquals(Collections.EMPTY_MAP, EventDataUtils.clone(map, true));
        } catch (Exception e) {
            fail("Should not throw exception");
        }

        // Unsupported types exist so should throw exception
        Exception ex =
                assertThrows(CloneFailedException.class, () -> EventDataUtils.clone(map, false));
        assertEquals(ex.getMessage(), "Object is of unsupported type");
    }

    @Test
    public void testClone_UnsupportedTypesMixedWithSupportedTypes_SkipUnsupported() {
        class Data {}
        ...
        try {
            ...
            expectedMap.put("string", "hello");
            expectedMap.put("integer", 1);
            assertEquals(expectedMap, EventDataUtils.clone(map, true));
        } catch (Exception e) {
            fail("Should not throw exception");
        }
    ...
    @Test
    public void testCloneFailCircularReference_SkipUnsupported() {
        Map<String, Object> map = new HashMap<>();
        List<Object> list = new ArrayList<>();
        list.add(map);
        ...
        Exception ex =
                assertThrows(CloneFailedException.class, () -> EventDataUtils.clone(map, true));
